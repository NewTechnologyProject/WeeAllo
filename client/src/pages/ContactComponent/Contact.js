  ...
  useEffect(() => {
    ...
    socket.current.on("send", (data) => {
      if (data.userReceive === Number(user)) {
        setNotify({
          isOpen: true,
          message: `Bạn đã nhận một lời mời kết bạn từ! ${data.userSend}`,
          type: "success",
        });
  ...
  useEffect(() => {
    ...
    socket.current.on("accept", (data) => {
      if (data.userSend === Number(user)) {
        setNotify({
          isOpen: true,
          message: `${data.userReceive} đã chấp nhận lời mời kết bạn của bạn`,
          type: "success",
        });
      }
      console.log(data);
          ...
          <TabContext value={value}>
            <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
              ...
              >
                ...
                <Tab
                  label={"Tất cả bạn bè " + "( " + allContactTab.length + " )"}
                  value="2"
                ></Tab>
                <Tab
                  label={"Các lời mời đã gửi " + "( " + allSend.length + " )"}
                  value="3"
                />
                <Tab
                  label={
                    "Các lời mời đã nhận" + "( " + allReceive.length + " )"
                  }
                  value="4"
                />
              </TabList>
